...
`)
	...
	options, _, err := loadStartupOptions([]string{
		...
		return []string{
			"NWAFU_CONNECT_SERVER_ADDRESS=env.example.com",
			"NWAFU_CONNECT_SERVER_PORT=555",
			"NWAFU_CONNECT_USERNAME=env-user",
		}
	})
	if err != nil {
		t.Fatal(err)
...
`)
	...
	options, _, err := loadStartupOptions([]string{"--auth-info"}, func() []string {
		return []string{"NWAFU_CONNECT_CONFIG=" + configFile}
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnknownConfigurationKeyIsRejected(t *testing.T) {
	...
	if _, _, err := loadStartupOptions(nil, func() []string {
		return []string{"NWAFU_CONNECT_SERVER_ADRESS=typo.example.com"}
	}); err == nil {
...
func TestLegacyRemoteDNSNamesRemainSupported(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "environment",
			environ: []string{
				"NWAFU_CONNECT_DISABLE_ZJU_DNS=true",
				"NWAFU_CONNECT_ZJU_DNS_SERVER=192.0.2.2",
			},
			wantDNS: "192.0.2.2",
		},
		{
...
`)
	...
	options, _, err := loadStartupOptions([]string{
		...
		return []string{
			"NWAFU_CONNECT_DISABLE_REMOTE_DNS=true",
			"NWAFU_CONNECT_ZJU_DNS_SERVER=192.0.2.2",
		}
	})
	if err != nil {
